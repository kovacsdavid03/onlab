...
def T_spoken_languages():
    ...
    logger.info(f"Exploding column 'spoken_languages': rows before explode = {df.shape[0]}.")
    df_exploded = df.explode('spoken_languages')
    logger.info(
        f"Exploded column 'spoken_languages' into fields [language]: rows after explode = {df_exploded.shape[0]} (delta = {df_exploded.shape[0] - df.shape[0]})."
    )
    ...
    logger.info(f"DataFrame 'temp_spoken_languages' has {df_final.shape[0]} rows and {df_final.shape[1]} columns.")
    df_final.to_sql('temp_spoken_languages', con=db_conn(), if_exists='append', index=False)
    logger.info(f"DataFrame written to SQL table 'temp_spoken_languages' with {df_final.shape[0]} rows.")